...
#include "BailingErrorStrategy.h"
#include "BailSimpleLexer.h"
...
int main(int argc, char* argv[])
{
  try
  {
    ...
    BailSimpleLexer lex(input.get());
    CommonTokenStream tokens(&lex);
    SimpleParser parser(&tokens);
    std::shared_ptr<BailingErrorStrategy> bes = std::make_shared<BailingErrorStrategy>();
    parser.setErrorHandler(bes);
